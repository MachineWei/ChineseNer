...
from models.electra_for_ner import ElectraSoftmaxForNer
...
MODEL_CLASSES = {
                'electra': (ElectraConfig, ElectraSoftmaxForNer, ElectraTokenizer),
...
def evaluate(args, model, tokenizer, label_list, pad_token_label_id):
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        with torch.no_grad():
            ...
            batch_predict_labels = logits.detach().cpu().numpy()
            batch_predict_labels = np.argmax(batch_predict_labels, axis=2)
        ...
        for index, input_length in enumerate(batch[4]):
            ...
            batch_predict = [args.id2label.get(i) for i in batch_predict_labels[index][:input_length]][1:-1]
...
def predict(args, model, tokenizer, label_list, pad_token_label_id, prefix=""):
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            ...
            batch_predict_labels = logits.detach().cpu().numpy()
            batch_predict_labels = np.argmax(batch_predict_labels, axis=2)
        batch_predict_labels = batch_predict_labels[0][1:-1]    # [CLS]XXXX[SEP]        
        ...
        ifError=False
        for input_id,pre,lab in zip(input_ids, batch_predict_labels, batch_true_labels):
        ...
        pbar(step)
        # 计算测试集 acc, recall, f1
        batch_true = [args.id2label.get(i) for i in batch_predict_labels]
        batch_predict = [args.id2label.get(i) for i in batch_true_labels]
