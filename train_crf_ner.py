...
def train(args, train_dataset, model, config, tokenizer, label_list, pad_token_label_id):
    """ Train the model """
    # 载入数据
...
def predict(args, model, tokenizer, label_list, pad_token_label_id, prefix=""):
    ...
    test_dataloader = DataLoader(test_dataset, sampler=test_sampler, batch_size=1, collate_fn=collate_fn)  # 每次只有一条数据
    # Eval
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        batch_predict_labels = batch_predict_labels[0][1:-1]    # [CLS]XXXX[SEP] 每次只有一条数据
...
def main():
    ...
    if args.do_train:
        ...
        global_step, lr_loss = train(args, train_dataset, model, config, tokenizer, label_list, pad_token_label_id)
